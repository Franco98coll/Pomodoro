<template>
    <div>
        ...
        <v-list-item prepend-icon="mdi-check-circle-outline" title="Tareas" @click="dialogTareas"></v-list-item>

        <!-- Define el diálogo de tareas -->
        <v-dialog v-model="isDialogActive" width="80%">
            <v-card class="d-flex justify-center align-center card">
                <v-btn v-if="UserId" @click="abrirAparte"><v-icon>mdi-open-in-new</v-icon>Abrir Aparte</v-btn>
                <TareasComp v-if="UserId" :UserId="UserId" />
                <v-card-text class="d-flex justify-center algin-center flex-column" v-else>
                    <h3>Para Acceder a las Tareas deberas loguearte</h3>
                    <div class="d-flex justify-center algin-center mt-5">
                        <v-btn @click="isDialogActive = False">
                            Aceptar
                        </v-btn>
                    </div>
...
watch(() => props.idDelUsuario, (newValue, oldValue) => {
